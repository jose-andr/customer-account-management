...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Publish Customer Account Management Markdown pages "
            "to Confluence."
        )
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show the publishing plan without changing Confluence.",
    )

    parser.add_argument(
        "--folder",
        choices=PUBLISHABLE_DIRECTORIES,
        help="Publish only one approved repository folder.",
    )
...
def discover_markdown_files(
    ...
    """Discover Markdown files in publishing order."""
    directories = (
        (selected_folder,)
        if selected_folder
    ...
    for directory_name in directories:
        ...
        if not directory.exists():
            print(f"Skipped missing folder: {directory_name}")
            continue
...
def humanise_name(value: str) -> str:
    """Convert a repository file or folder name into a page title."""
    cleaned = re.sub(r"^\d{2}-", "", value)
    ...
    for word in cleaned.split():
        ...
        words.append(replacement or word.capitalize())
...
def extract_page_title(
    ...
    return humanise_name(file_path.stem)
...
def parse_git_history(path: Path) -> list[CommitRecord]:
    ...
    output = run_git_command(
        [
            "log",
            "--follow",
            f"--max-count={REVIEW_NOTES_LIMIT}",
...
def inject_review_notes(
    ...
    base_content = MANAGED_REVIEW_PATTERN.sub(
        "",
        markdown_content,
    ).rstrip()

    review_notes = build_review_notes(commits)
...
def build_folder_body(
    ...
    folder_title = humanise_name(folder_path.name)
...
def publish_repository(
    *,
    ...
    for folder_path in required_folders:
        repository_parent = folder_path.parent
        ...
        folder_page = client.create_or_update_page(
            title=humanise_name(folder_path.name),
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"Discovered {len(markdown_files)} "
            "publishable Markdown file(s)."
        )

        client = ConfluenceClient(configuration)
        client.load_pages()
        ...
        print(
            f"Confluence publishing failed: {error}",
            file=sys.stderr,
        )
        return 1
